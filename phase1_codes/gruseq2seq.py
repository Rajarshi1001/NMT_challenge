...
class Encoder(nn.Module):
    ...
    - gru (nn.GRU): Gated Recurrent Unit to process the embedded sequences.
...
class Decoder(nn.Module):
    ...
    - embedding (nn.Embedding): Transforms input tokens into dense vectors.
    - gru (nn.GRU): Gated Recurrent Unit to process the embedded sequences.
    - ll (nn.Linear): Linear layer to transform the GRU outputs to predicted tokens.
    - dropout (nn.Dropout): Regularization layer to prevent overfitting.
    """
...
class GRUSeq2Seq(nn.Module):
    """
    A Sequence-to-Sequence network using GRU as the recurrent unit. This module 
