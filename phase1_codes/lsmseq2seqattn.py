...
from gruseq2seq import Encoder, Decoder, GRUSeq2Seq
...
model = LSTMeq2Seq(encoder, decoder).to(device)
...
def evaluate(language):
    ...
    data.to_csv(os.path.join("ls_translations", "answer_{}_test.csv".format(language)))
